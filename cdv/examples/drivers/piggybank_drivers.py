...
# Create a piggybank
def create_piggybank_puzzle(amount: uint64, cash_out_puzhash: bytes32):
    return PIGGYBANK_MOD.curry(amount, cash_out_puzhash)

# Generate a solution to contribute to a piggybank
def solution_for_piggybank(pb_coin: Coin, contrib_amount: uint64):
...
def piggybank_announcement_assertion(pb_coin: Coin, contrib_amount: uint64):
    return [ConditionOpcode.ASSERT_COIN_ANNOUNCEMENT, std_hash(pb_coin.name() + int_to_bytes(pb_coin.amount + contrib_amount))]